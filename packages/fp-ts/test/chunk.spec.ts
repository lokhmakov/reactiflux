...
describe.concurrent('Chunk', () => {
  ...
  describe('fromIterable', () => {
    describe('Given an iterable', () => {
      ...
      it('should process it', () => {
        expect(C.fromIterable(myIterable)).toEqual(
          C.unsafeFromArray([1, 2, 3, 4, 5]),
        )
      })
    })
  })
  ...
  describe('get', () => {
    describe('Given a Chunk and an index within the bounds', () => {
      ...
      it('should a Some with the value', () => {
        expect(pipe(chunk, C.get(index))).toEqual(O.some(1))
      })
    })
      ...
      it('should throw', () => {
        fc.assert(
          fc.property(
            ...
            fc.array(fc.anything()),
            (arr1, arr2) => {
              const chunk: C.Chunk<unknown> = C.concat(C.fromIterable(arr2))(
                C.unsafeFromArray(arr1),
              )
              expect(() =>
                pipe(chunk, C.unsafeGet(arr1.length + arr2.length)),
              ).toThrow()
            },
          ),
        )
  ...
  describe('take', () => {
    describe('Given a Chunk with more elements than the amount taken', () => {
      it('should return the subset', () => {
        expect(pipe(C.unsafeFromArray([1, 2, 3]), C.take(2))).toEqual(
          C.unsafeFromArray([1, 2]),
        )
      })
    })
    ...
    describe('Given a Chunk with fewer elements than the amount taken', () => {
      ...
      it('should return the available subset', () => {
        expect(pipe(chunk, C.take(amount))).toEqual(
          C.unsafeFromArray([1, 2, 3]),
        )
      })
    })
    ...
    describe('Given a singleton Chunk with and an amount > 1', () => {
      ...
      it('should return the available subset', () => {
        expect(pipe(chunk, C.take(amount))).toEqual(C.unsafeFromArray([1]))
      })
    })
  })
  ...
  describe('dropRight', () => {
    describe('Given a Chunk and an amount to drop below the length', () => {
      ...
      it('should remove the given amount of items', () => {
        expect(pipe(chunk, C.dropRight(toDrop))).toEqual(
          C.unsafeFromArray([1, 2]),
        )
      })
    })
  ...
  describe('dropWhile', () => {
    describe('Given a Chunk and a criteria that applies to part of the chunk', () => {
      ...
      it("should return the subset that doesn't pass the criteria", () => {
        expect(pipe(chunk, C.dropWhile(criteria))).toEqual(
          C.unsafeFromArray([3]),
        )
      })
    })
  ...
  describe('concat', () => {
    describe('Given 2 chunks of the same length', () => {
      ...
      it('should concatenate them following order', () => {
        expect(pipe(chunk1, C.concat(chunk2))).toEqual(
          C.unsafeFromArray([0, 1, 2, 3]),
        )
      })
    })
    ...
    describe('Given 2 chunks where the first one is appended', () => {
      const chunk1 = pipe(C.empty, C.append(1))
      const chunk2 = C.unsafeFromArray([2, 3, 4])
    ...
    describe('Given 2 chunks where the second one is appended', () => {
      const chunk1 = C.unsafeFromArray([1])
      const chunk2 = pipe(C.empty, C.prepend(2))
  ...
  it('zip', () => {
    pipe(C.empty, C.zip(C.empty), equals(C.unsafeFromArray([])), assert.isTrue)
    pipe(
      C.empty,
      C.zip(C.singleton(1)),
  ...
  it('zipWithIndex', () => {
    pipe(C.empty, C.zipWithIndex, equals(C.unsafeFromArray([])), assert.isTrue)
    pipe(
      C.unsafeFromArray([1, 2, 3, 4]),
      C.zipWithIndex,
      equals(
  ...
  it('zipWithIndexOffset', () => {
    pipe(C.empty, C.zipWithIndexOffset(5), equals(C.unsafeFromArray([])))
    pipe(
      C.unsafeFromArray([1, 2, 3, 4]),
