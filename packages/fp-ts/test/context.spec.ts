...
describe.concurrent('Context', () => {
  it('adds and retrieve services', () => {
    ...
    assert.isTrue(pipe(Services, Context.get(A), equals({a: 0})))

    assert.isTrue(pipe(Services, Context.getOption(B), equals(O.some({b: 1}))))

    assert.isTrue(pipe(Services, Context.getOption(C), equals(O.none)))

    assert.throw(() => {
      pipe(Services, Context.unsafeGet(C))
    })
  })
  ...
  it('prunes services in env and merges', () => {
    const env = pipe(
      ...
      Context.add(A)({a: 0}),
      Context.merge(
        pipe(Context.empty(), Context.add(B)({b: 1}), Context.add(C)({c: 2})),
      ),
    )

    const pruned = pipe(env, Context.prune(A, B))

    assert.isTrue(pipe(pruned, Context.get(A), equals({a: 0})))

    assert.isTrue(pipe(pruned, Context.getOption(B), equals(O.some({b: 1}))))

    assert.isTrue(pipe(pruned, Context.getOption(C), equals(O.none)))

    assert.isTrue(pipe(env, Context.getOption(C), equals(O.some({c: 2}))))
  })
